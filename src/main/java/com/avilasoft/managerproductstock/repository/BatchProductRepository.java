...
public interface BatchProductRepository extends JpaRepository<BatchProduct, Long> {
    ...
    @Query(
        value = "select distinct batchProduct from BatchProduct batchProduct left join fetch batchProduct.batch",
    ...
    @Query("select distinct batchProduct from BatchProduct batchProduct left join fetch batchProduct.batch")
    List<BatchProduct> findAllWithToOneRelationships();

    @Query("select batchProduct from BatchProduct batchProduct left join fetch batchProduct.batch where batchProduct.id =:id")
