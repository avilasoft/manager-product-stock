...
public interface BatchRepository extends JpaRepository<Batch, Long> {
    ...
    @Query(
        value = "select distinct batch from Batch batch left join fetch batch.bachStatus left join fetch batch.businessAssociate left join fetch batch.batchProduct",
        countQuery = "select count(distinct batch) from Batch batch"
    )
    Page<Batch> findAllWithToOneRelationships(Pageable pageable);

    @Query(
        "select distinct batch from Batch batch left join fetch batch.bachStatus left join fetch batch.businessAssociate left join fetch batch.batchProduct"
    )
    List<Batch> findAllWithToOneRelationships();

    @Query(
        "select batch from Batch batch left join fetch batch.bachStatus left join fetch batch.businessAssociate left join fetch batch.batchProduct where batch.id =:id"
    )
