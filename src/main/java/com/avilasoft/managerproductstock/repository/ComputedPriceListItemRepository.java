...
public interface ComputedPriceListItemRepository extends JpaRepository<ComputedPriceListItem, Long> {
    ...
    @Query(
        value = "select distinct computedPriceListItem from ComputedPriceListItem computedPriceListItem left join fetch computedPriceListItem.unitPriceList",
    ...
    @Query(
        "select distinct computedPriceListItem from ComputedPriceListItem computedPriceListItem left join fetch computedPriceListItem.unitPriceList"
    )
    List<ComputedPriceListItem> findAllWithToOneRelationships();

    @Query(
        "select computedPriceListItem from ComputedPriceListItem computedPriceListItem left join fetch computedPriceListItem.unitPriceList where computedPriceListItem.id =:id"
    )
