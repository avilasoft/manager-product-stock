...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    @Query(
        value = "select distinct product from Product product left join fetch product.unit left join fetch product.productCategory",
        countQuery = "select count(distinct product) from Product product"
    )
    Page<Product> findAllWithToOneRelationships(Pageable pageable);

    @Query("select distinct product from Product product left join fetch product.unit left join fetch product.productCategory")
    List<Product> findAllWithToOneRelationships();

    @Query("select product from Product product left join fetch product.unit left join fetch product.productCategory where product.id =:id")
