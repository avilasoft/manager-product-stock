...
public interface ProviderProductPriceRepository extends JpaRepository<ProviderProductPrice, Long> {
    ...
    @Query(
        value = "select distinct providerProductPrice from ProviderProductPrice providerProductPrice left join fetch providerProductPrice.price",
    ...
    @Query("select distinct providerProductPrice from ProviderProductPrice providerProductPrice left join fetch providerProductPrice.price")
    List<ProviderProductPrice> findAllWithToOneRelationships();

    @Query(
        "select providerProductPrice from ProviderProductPrice providerProductPrice left join fetch providerProductPrice.price where providerProductPrice.id =:id"
    )
