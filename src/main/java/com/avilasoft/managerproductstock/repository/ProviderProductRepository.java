...
public interface ProviderProductRepository extends JpaRepository<ProviderProduct, Long> {
    ...
    @Query(
        value = "select distinct providerProduct from ProviderProduct providerProduct left join fetch providerProduct.provider left join fetch providerProduct.providerProductPrice",
    ...
    @Query(
        "select distinct providerProduct from ProviderProduct providerProduct left join fetch providerProduct.provider left join fetch providerProduct.providerProductPrice"
    )
    List<ProviderProduct> findAllWithToOneRelationships();

    @Query(
        "select providerProduct from ProviderProduct providerProduct left join fetch providerProduct.provider left join fetch providerProduct.providerProductPrice where providerProduct.id =:id"
    )
