...
public interface ProviderProductRepository extends JpaRepository<ProviderProduct, Long> {
    ...
    @Query(
        value = "select distinct providerProduct from ProviderProduct providerProduct left join fetch providerProduct.product left join fetch providerProduct.batchProduct left join fetch providerProduct.provider",
    ...
    @Query(
        "select distinct providerProduct from ProviderProduct providerProduct left join fetch providerProduct.product left join fetch providerProduct.batchProduct left join fetch providerProduct.provider"
    )
    List<ProviderProduct> findAllWithToOneRelationships();

    @Query(
        "select providerProduct from ProviderProduct providerProduct left join fetch providerProduct.product left join fetch providerProduct.batchProduct left join fetch providerProduct.provider where providerProduct.id =:id"
    )
