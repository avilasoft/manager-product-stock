...
public interface ProviderRepository extends JpaRepository<Provider, Long> {
    ...
    @Query(
        value = "select distinct provider from Provider provider left join fetch provider.businessAssociate",
    ...
    @Query("select distinct provider from Provider provider left join fetch provider.businessAssociate")
    List<Provider> findAllWithToOneRelationships();

    @Query("select provider from Provider provider left join fetch provider.businessAssociate where provider.id =:id")
