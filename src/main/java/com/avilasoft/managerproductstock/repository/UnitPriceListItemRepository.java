...
public interface UnitPriceListItemRepository extends JpaRepository<UnitPriceListItem, Long> {
    ...
    @Query(
        value = "select distinct unitPriceListItem from UnitPriceListItem unitPriceListItem left join fetch unitPriceListItem.projectItemListItem left join fetch unitPriceListItem.unitPriceList",
    ...
    @Query(
        "select distinct unitPriceListItem from UnitPriceListItem unitPriceListItem left join fetch unitPriceListItem.projectItemListItem left join fetch unitPriceListItem.unitPriceList"
    )
    List<UnitPriceListItem> findAllWithToOneRelationships();

    @Query(
        "select unitPriceListItem from UnitPriceListItem unitPriceListItem left join fetch unitPriceListItem.projectItemListItem left join fetch unitPriceListItem.unitPriceList where unitPriceListItem.id =:id"
    )
