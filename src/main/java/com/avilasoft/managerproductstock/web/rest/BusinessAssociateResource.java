...
public class BusinessAssociateResource {
    ...
    public ResponseEntity<List<BusinessAssociate>> getAllBusinessAssociates(
        ...
        if ("provider-is-null".equals(filter)) {
            log.debug("REST request to get all BusinessAssociates where provider is null");
            return new ResponseEntity<>(businessAssociateService.findAllWhereProviderIsNull(), HttpStatus.OK);
        }

        if ("project-is-null".equals(filter)) {
            log.debug("REST request to get all BusinessAssociates where project is null");
            return new ResponseEntity<>(businessAssociateService.findAllWhereProjectIsNull(), HttpStatus.OK);
        }
