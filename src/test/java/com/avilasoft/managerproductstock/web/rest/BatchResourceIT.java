...
class BatchResourceIT {
    ...
    public static Batch createEntity(EntityManager em) {
        ...
        if (TestUtil.findAll(em, BatchProduct.class).isEmpty()) {
            batchProduct = BatchProductResourceIT.createEntity(em);
            em.persist(batchProduct);
            em.flush();
        } else {
            batchProduct = TestUtil.findAll(em, BatchProduct.class).get(0);
        }
    ...
    public static Batch createUpdatedEntity(EntityManager em) {
        ...
        BatchProduct batchProduct;
        if (TestUtil.findAll(em, BatchProduct.class).isEmpty()) {
            batchProduct = BatchProductResourceIT.createUpdatedEntity(em);
            em.persist(batchProduct);
            em.flush();
        } else {
            batchProduct = TestUtil.findAll(em, BatchProduct.class).get(0);
