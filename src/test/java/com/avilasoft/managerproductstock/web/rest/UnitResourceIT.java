...
class UnitResourceIT {
    ...
    public static Unit createEntity(EntityManager em) {
        Unit unit = new Unit().name(DEFAULT_NAME).description(DEFAULT_DESCRIPTION).symbol(DEFAULT_SYMBOL).isBase(DEFAULT_IS_BASE);
        // Add required entity
        UnitType unitType;
        if (TestUtil.findAll(em, UnitType.class).isEmpty()) {
            unitType = UnitTypeResourceIT.createEntity(em);
            em.persist(unitType);
            em.flush();
        } else {
            unitType = TestUtil.findAll(em, UnitType.class).get(0);
        }
    ...
    public static Unit createUpdatedEntity(EntityManager em) {
        Unit unit = new Unit().name(UPDATED_NAME).description(UPDATED_DESCRIPTION).symbol(UPDATED_SYMBOL).isBase(UPDATED_IS_BASE);
        // Add required entity
        UnitType unitType;
        if (TestUtil.findAll(em, UnitType.class).isEmpty()) {
            unitType = UnitTypeResourceIT.createUpdatedEntity(em);
            em.persist(unitType);
            em.flush();
        } else {
            unitType = TestUtil.findAll(em, UnitType.class).get(0);
    ...
    void createUnit() throws Exception {
        ...
        assertThat(testUnit.getIsBase()).isEqualTo(DEFAULT_IS_BASE);
    }
    ...
    void putNewUnit() throws Exception {
        ...
        updatedUnit.name(UPDATED_NAME).description(UPDATED_DESCRIPTION).symbol(UPDATED_SYMBOL).isBase(UPDATED_IS_BASE);
        ...
        assertThat(testUnit.getIsBase()).isEqualTo(UPDATED_IS_BASE);
    }
    ...
    void fullUpdateUnitWithPatch() throws Exception {
        ...
        partialUpdatedUnit.name(UPDATED_NAME).description(UPDATED_DESCRIPTION).symbol(UPDATED_SYMBOL).isBase(UPDATED_IS_BASE);
